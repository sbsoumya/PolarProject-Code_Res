...
def send_rateless_det(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,list_size): 
	...
	pc1=ec.polarcode_init_defch(N,Iter_G,Iter_p,I_ord,0) #Iter_p is a dummy design_p
	...
	while not decoded :
		...
		#Iter_UN_hat=ec.polarSCdecodeG_C(Iter_YN,N,Iter_p,Iter_I,list(Iter_D),False)	(pc1,YN,decode_p,D,list_size)	
	...
	for Iter in range(final_Iter-1,-1,-1):
			...
			pc1.info_length=final_G
			Iter_UN_decoded=ec.polarIncFrzSCdecodeG_C(pc1,Iter_YN,final_p,list(Iter_D),IncFreeze_ind,IncFreeze_data,list_size)
...
#R R/2 R/3 R/4.....		
def send_rateless_det_sim(N,T,compound_plist_u,channel_p,msg_length,runsim,list_size): #using bestchannel sent rate in place of derate
	...
	for i in range(runsim):
		...
		(achievedrate_sim,Iter,UN_msg_decoded)=send_rateless_det(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,list_size)
...
def send_rateless_det_Iter_retro(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,list_size): 
...
def send_rateless_det_Iter_retro_sim(N,T,compound_plist_u,channel_p,msg_length,runsim,list_size): #using bestchannel sent rate in place of derate
	...
	for i in range(runsim):
		UN_msg=np.random.randint(2,size=Glist[0]-T)
		(achievedrate_sim,Iter,UN_msg_decoded)=send_rateless_det_Iter_retro(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,list_size)
		achievedrate+=float(achievedrate_sim)/runsim
		if UN_msg.tolist()!=UN_msg_decoded.tolist():
			block_errorcnt+=1
		try:
...
def send_rateless_det_Iter_retro_UK(UN_msg,N,I_ord,channel_p,compound_plist,Glist): 
	...
	while not decoded :
		...
		if is_mismatch(list(Iter_UN_retro_decoded),list(UN_msg)) and Iter<maxiter:
	...
	#final==================================
	final_Iter=Iter
	if is_mismatch(list(Iter_UN_retro_decoded),list(UN_msg)): # two find the cases where final iter did not send ACK
		return_iter=0
	else:
		return_iter=final_Iter+1
	final_decoded=Iter_UN_retro_decoded
...
def send_rateless_det_Iter_retro_UK_sim(N,compound_plist_u,channel_p,msg_length,runsim): #using bestchannel sent rate in place of derate
	T=0
	compound_plist=list(compound_plist_u) #best channel first
	compound_plist.sort()
	I_ord=pcon.getreliability_order(N)
	...
	for i in range(runsim):
		UN_msg=np.random.randint(2,size=Glist[0]-T)
		(achievedrate_sim,Iter,UN_msg_decoded)=send_rateless_det_Iter_retro_UK(UN_msg,N,I_ord,channel_p,compound_plist,Glist)
		achievedrate+=float(achievedrate_sim)/runsim
		if UN_msg.tolist()!=UN_msg_decoded.tolist():
			block_errorcnt+=1
		try:
