#-------------------------------------------------------------------------------
# Name:       polarfilesim_FERvsR_derate_rateless_Irv.py
...
LTPTdict={'0.04':[5,15],'0.15':[5,5],'0.2':[5,5],'0.25':[0,0]}
design_p=0.04
...
for N in Nlist:
	for channel_p in channel_plist:
		...
		f1=open("./simresults/polarfile_FERvsR_derate_rateless_Irv"+str(N)+"_"+str(channel_p)+"_"+stamp+".txt",'w')
			
		print "RATE Vs FER REPORT FILE Rateless Irv"
		print "-------------------------------------"
		print "N="+str(N)
		...
		json.dump( "RATE Vs FER REPORT FILE Rateless Irv",f1) ;f1.write("\n")
		...
		for derate in deratelist:
			(ach_rate,u_rate,block_error)=rlf.send_rateless_file_Irv_sim(N,compound_plist,channel_p,derate,LTPTdict,runsim,False)
