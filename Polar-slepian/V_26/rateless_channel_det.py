...
#=============================Asymmetric channel
def send_rateless_det_Iter_retro_p01(UN_msg,N,T,I_ord,channel_p0,channel_p1,compound_plist,Glist): 
	...
	while not decoded :
		...
		#--------------------Note channel_p used for flipping
		Iter_YN=pl.BACN(channel_p0,channel_p1,Iter_XN)
...
def send_rateless_det_Iter_retro_p01_sim(N,T,compound_plist_u,channel_p0,channel_p1,msg_length,runsim): #using bestchannel sent rate in place of derate
	...
	achievedrate=0
	print "msg_length:"+str(Glist[0]-T)
	print "channel_p0:"+str(channel_p0)
	print "channel_p1:"+str(channel_p1)
	
	for i in range(runsim):
		UN_msg=np.random.randint(2,size=Glist[0]-T)
		(achievedrate_sim,Iter,UN_msg_decoded)=send_rateless_det_Iter_retro_p01(UN_msg,N,T,I_ord,channel_p0,channel_p1,compound_plist,Glist)
		achievedrate+=float(achievedrate_sim)/runsim
		if UN_msg.tolist()!=UN_msg_decoded.tolist():
			block_errorcnt+=1
		try:
