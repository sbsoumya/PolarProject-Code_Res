...
filename="./simresults/polarfile_FERvsp_FR"+str(msg_length)+"in"+str(N)+"_"+stamp+".txt"
f1=open(filename,'w')
print filename
print "P Vs FER REPORT file"
print "---------------------------"
print "N="+str(N)
...
for design_p in design_plist:
	print design_p
	block_error=pf.polarfilesim_FR(N,design_p,design_p,msg_length,runsim,False)
	#~ if block_error==0:
		#block_error=pch.polarchannelsim_FR(N,design_p,design_p,msg_length,runsim,False)
	block_errorL=pf.polarfilesim_FR_list(N,design_p,design_p,msg_length,runsim,False,L)
