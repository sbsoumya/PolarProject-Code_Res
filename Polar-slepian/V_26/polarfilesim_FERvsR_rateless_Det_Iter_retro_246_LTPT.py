...
for N in Nlist:
	...
	filename="./simresults/polarfile_FERvsR_rateless_Det_LTPT_"+str(R_p1)+"in"+str(N)+"_T"+str(T)+"_"+stamp+".txt"
	f1=open(filename,'w')
	print filename
	...
	json.dump( "RATE Vs FER REPORT Rateless Det Iter LTPT",f1) ;f1.write("\n")
	...
	for channel_p in channel_plist:
		...
		(u_rate,ach_rate,block_error,Iter_probdict)=rlf.send_rateless_file_LTPT_new_sim(N,LTPTdict,compound_plist,channel_p,error_free_msg_length,runsim)
