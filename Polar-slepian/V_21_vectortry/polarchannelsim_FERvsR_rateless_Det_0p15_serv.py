...
T=20
msg_lengthlist=range(50,150,50)
...
for N in Nlist:
	for channel_p in channel_plist:
		...
		f1=open("./simresults/polarchannel_FERvsR_rateless_Det"+str(N)+"_"+str(channel_p)+"_"+stamp+".txt",'w')
			
		print "RATE Vs FER REPORT Rateless Det"
		print "--------------------------------"
		print "N="+str(N)
		...
		json.dump( "RATE Vs FER REPORT Rateless Det",f1) ;f1.write("\n")
		...
		for msg_length in msg_lengthlist:
			...
			(u_rate,ach_rate,block_error,Iter_probdict)=rlc.send_rateless_det_sim(N,T,compound_plist,channel_p,msg_length,runsim)
