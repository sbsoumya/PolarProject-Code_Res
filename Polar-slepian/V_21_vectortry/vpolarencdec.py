...
#---------------------------------------------------recursive encoder	
def vpolarencodeG(UN,N,I,D,isort): 
	...
	VN=[]
	UNx=[list(un) for un in UN] #as pop is a destructive readout
	
	if len(UN[0])==len(I) and len(D[0])==N-len(I):
		if isort:
			VN=[formVN_s(unx,N,I,d) for (unx,d) in (UNx,D)]
		else:
			VN=[formVN_u(unx,N,I,d) for (unx,d) in zip(UNx,D)]
   	else:
		print "UN length"
		print len(UN[0])
		print "G"
