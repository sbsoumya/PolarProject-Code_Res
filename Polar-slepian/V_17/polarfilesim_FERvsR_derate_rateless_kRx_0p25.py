#-------------------------------------------------------------------------------
# Name:       polarfilesim_FERvsR_derate_rateless.py
# Purpose:    FER VS R simulation for given P and different rates
#             for rateless_kRx file
#
# Author:      soumya
#
# Created:    28/12/2017
...
for N in Nlist:
	for channel_p in channel_plist:
		...
		f1=open("./simresults/polarfile_FERvsR_derate_rateless_kRx"+str(N)+"_"+str(channel_p)+"_"+stamp+".txt",'w')
			
		print "RATE Vs FER REPORT FILE Rateless kRx"
		print "-------------------------------------"
		print "N="+str(N)
		...
		json.dump( "RATE Vs FER REPORT FILE Rateless kRx",f1) ;f1.write("\n")
		...
		for derate in deratelist:
			(ach_rate,u_rate,block_error)=rlf.send_rateless_file_kRx_sim(N,compound_plist,channel_p,derate,runsim,False)
