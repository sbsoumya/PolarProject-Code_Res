...
for ch in range(N):
	...
	for channel_p in channel_plist:
		...
		f_ch=[lmb.f_Irv(llr,int(sb)) for (llr,sb) in zip(llr_ch,sbit_ch)]
		sns.distplot(f_ch,label=str(channel_p)+"\n m="+str(np.mean(f_ch))+"\n v="+str(np.var(f_ch)))
		mean[str(channel_p)]=np.mean(f_ch)
		var[str(channel_p)]=np.var(f_ch)
	
	plt.title("f_Irv(LLR,b) for design_p=0.04, Rel_Order="+str(ch)+"/"+str(N)+" G="+str(G))
	plt.legend(loc="best")
	plt.savefig("./simresults/stats/f_Irv/f_Irv_stats_0p04"+"_"+str(ch)+".png", bbox_inches='tight')	
	plt.close()
