...
def send_rateless_file_Iter_retro_3G(XN,N,I_ord,channel_p1,channel_p2,compound_plist,Glist,T,printFT): 
	...
	return (Total_error_free,error,decoded,errorarray,Iter_prob,[D_X,D_Y,D_Z],Emp_comp_len)
...
def send_rateless_file_Iter_retro_det_3G_sim(N,T,compound_plist_u,channel_p1,channel_p2,error_free_msg_length,runsim,printFT):
	...
	for i in range(runsim):
		XN=np.random.randint(2,size=N)
		(Total_error_free,error,decoded,errorarray,Iter_prob,Darray,Emp_comp_len)=send_rateless_file_Iter_retro_3G(XN,N,I_ord,channel_p1,channel_p2,compound_plist_u,Glist,T,printFT)
		errorfree_ach_rate+=float(Total_error_free)/(N*runsim) # calculates E{D}/N
	...
	return (errorfree_ach_rate,block_error,decoded,error_array,Iter_probab,D_array,Emp_comp)
