...
print channel_plist
compound_plist=[0.03,0.11,0.17,0.2,0.23] #for Z< 5*0.01
...
for N in Nlist:
	...
	filename="./simresults/polarchannel_FERvsR_rateless_Det_Iter_retro_UK"+str(R_p1)+"in"+str(N)+"_T"+str(T)+"_"+str(int(maxiters))+"I_"+stamp+".txt"
	f1=open(filename,'w')
	print filename
	...
	for channel_p in channel_plist:
		...
		(u_rate,ach_rate,block_error,Iter_probdict)=rlc.send_rateless_det_Iter_retro_UK_sim(N,compound_plist,channel_p,msg_length,runsim)
