...
start = timer()
Nlist=[512]
channel_p0list=list(np.linspace(0.01,0.2,20))
channel_p1list=list(np.linspace(0.005,0.1,20))
print channel_p0list,channel_p1list
...
for N in Nlist:
	...
	f1=open(filename,'w')
	print filename
	print "RATE Vs FER REPORT Rateless Det Iter retro for p0"
	print "------------------------------------------"
	...
	for i in range(len(channel_p0list)):
		#print "channel_p:"+str(channel_p)
		(u_rate,ach_rate,block_error,Iter_probdict)=rlc.send_rateless_det_Iter_retro_p01_sim(N,T,compound_plist,channel_p0list[i],channel_p1list[i],msg_length,runsim)
...
json.dump(channel_p0list,f1) ;f1.write("\n")
json.dump(channel_p1list,f1) ;f1.write("\n")
