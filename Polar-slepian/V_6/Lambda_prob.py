...
N=32
channel_plist=[0.05,0.07,0.1,0.2,0.3,0.4]
channel_plist.sort()
...
lambda_threshold1=min(ec.getchannels(lambda_thresholdlist,I))
lambda_threshold2=max(ec.getchannels(lambda_thresholdlist,B))
...
for channel_p in channel_plist:
	...
	for i in range(runsim):
		...
		(llr,d)=ec.polarSCdecodeG_LLR(YN,N,design_p,I,list(D))
		L=abs(llr)
		llr_Gmin=min(ec.getchannels(L,I))
		llr_Fmax=max(ec.getchannels(L,B))
	...
	#print "specimen:"
	#print ec.getchannels(L,I)
	#print ec.getchannels(L,B)
