...
#===============================tpt 
fig=plt.figure()
plt.subplots_adjust(hspace=0.3,top=0.99)
ax=plt.subplot(311)
...
TPTANAp1=(NR_p1-np.array(TlistANA))*(1-FERp1)/(N*E_Iterp1)
plt.plot(TlistANA,TPTANAp1,"-.bo",label="Ana")
...
plt.plot(lines[0],lines[1],"-m^",label="Sim")
...
plt.plot(lines[0],lines[1],"-m^")
#plt.plot(lines[0][lines[1].index(max(lines[1]))],max(lines[1]),"go",label="max(TPT-sim)")
#plt.axvline(x=lines[0][lines[1].index(max(lines[1]))], color='b', linestyle='-.')
#plt.legend(loc="lower right")
plt.ylabel('$\eta(0.11)$')#, fontsize=20)
box = ax.get_position()
#ax.set_position([box.x0, box.y0 + box.height * 0.1, box.width, box.height * 0.8])
...
TPTANAp3=(NR_p1-np.array(TlistANA))*(1-FERp2)/(N*E_Iterp3)
plt.plot(TlistANA,TPTANAp3,"-.bo",label="$\eta$, ANA")
...
plt.plot(lines[0],lines[1],"-m^",label="$\eta$, SIM")
#plt.plot(lines[0][lines[1].index(max(lines[1]))],max(lines[1]),"go",label="max(TPT-sim)")
...
#~ FER1est=[estimateFER(TlistANA,N,NR_p1,p,1) for p in channel_plist]
#~ FER2est=[estimateFER(TlistANA,N,NR_p1,p,2) for p in channel_plist]
#~ FER3est=[estimateFER(TlistANA,N,NR_p1,p,3) for p in channel_plist]
