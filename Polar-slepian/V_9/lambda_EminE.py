# -------------------------------------------------------------------------------
# Name:       lambda_EmimE.py
# Purpose:    Find E min , min E of Lambda
#
# Author:      soumya
#
# Created:    24/10/2017
...
f1 = open("./simresults/lambda_EminE" + stamp + ".txt", 'w')
...
print len(B)
##########################################file
json.dump("Lambda_finder", f1);
f1.write("\n")
json.dump("---------------------------", f1);
f1.write("\n")
json.dump("N=" + str(N), f1);
f1.write("\n")
json.dump("design_p=" + str(design_p), f1);
f1.write("\n")
json.dump("Capacity for " + str(N) + "channels:" + str(PotGCh), f1);
f1.write("\n")

if not designbyrate:
    json.dump("tolerable error exponent:" + str(tolerable_error), f1);
    f1.write("\n")
else:
    json.dump("design for rate:" + str(K), f1);
    f1.write("\n")

json.dump("Z Construct--------------------------------------------------------", f1);
f1.write("\n")
json.dump("Good Channels:", f1);
f1.write("\n")
json.dump(I, f1);
f1.write("\n")
json.dump("Number of good channels:", f1);
f1.write("\n")
json.dump(len(I), f1);
f1.write("\n")
json.dump("R=" + str(R), f1);
f1.write("\n")
json.dump("Frozen channels:", f1);
f1.write("\n")
json.dump(len(B), f1);
f1.write("\n")

json.dump("sim ran for LT:" + str(runsim_th), f1);
f1.write("\n")
# =======================================================Simulation
# Frozen data
...
for channel_p in channel_plist:
    ...
    f1.write("\n")
    json.dump("Lambda Min(E(LLR))" + str(lambda_minE), f1);
    f1.write("\n")
    json.dump("Lambda E(min(LLR))" + str(lambda_Emin), f1);
    f1.write("\n")
