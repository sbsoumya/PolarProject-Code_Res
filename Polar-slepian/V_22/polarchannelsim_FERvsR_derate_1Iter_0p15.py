...
for N in Nlist:
	for design_p in design_plist:
		...
		f1=open("./simresults/polarchannel_FERvsR_derate_1Iter"+str(N)+"_"+str(design_p)+"_"+stamp+".txt",'w')
			
		print "RATE Vs FER REPORT 1Iter"
		print "---------------------------"
		print "N="+str(N)
		...
		json.dump( "RATE Vs FER REPORT 1Iter",f1) ;f1.write("\n")
		...
		for derate in deratelist:
				(ach_rate,u_rate,block_error)=rlc.send_rateless_kRx_sim(N,[design_p],design_p,derate,runsim,False)
