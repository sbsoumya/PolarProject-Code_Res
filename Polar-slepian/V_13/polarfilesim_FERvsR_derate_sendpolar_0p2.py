#-------------------------------------------------------------------------------
# Name:       polarfilesim_FERvsR_sendpolar.py
# Purpose:    This uses the rateless sendpolar (with one iter)
...
#Nlist=[2048]
#Nlist=[4096]
design_plist=[0.2]#0.15,0.2,0.25]
...
for N in Nlist:
	for design_p in design_plist:
		...
		f1=open("./simresults/polarfile_FERvsR_derate_sendpolar"+str(N)+"_"+str(design_p)+"_"+stamp+".txt",'w')
			
		print "RATE Vs FER REPORT FILE send polar"
		print "----------------------------------"
		print "N="+str(N)
		...
		json.dump( "RATE Vs FER REPORT FILE send polar",f1) ;f1.write("\n")
		...
		for derate in deratelist:
				(ach_rate,u_rate,block_error)=rlf.send_polarfile_sim(N,[design_p],design_p,derate,runsim,False)
