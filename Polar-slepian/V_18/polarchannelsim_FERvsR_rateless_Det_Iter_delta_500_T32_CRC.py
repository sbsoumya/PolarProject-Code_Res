...
for N in Nlist:
	...
	filename="./simresults/polarchannel_FERvsR_rateless_Det_Iter_delta_CRC"+str(msg_length)+"in"+str(N)+"_T"+str(T)+"_"+stamp+".txt"
	f1=open(filename,'w')
	print filename
	print "RATE Vs FER REPORT Rateless Det Iter delta CRC"
	print "------------------------------------------"
	...
	json.dump( "RATE Vs FER REPORT Rateless Det Iter delta CRC",f1) ;f1.write("\n")
	...
	for channel_p in channel_plist:
		...
		(u_rate,ach_rate,block_error,Iter_probdict)=rlc.send_rateless_det_Iter_retro_delta_CRC_sim(N,T,compound_plist,channel_p,msg_length,deltaG,runsim)
