...
#DO ITER FUNCTIONS RUN FOR SPECIFIED NUMBER OF ITERATIONS ONLY
#=======================================================================Detectionbits	

def is_mismatch(lock,key):	
	return lock!=key
	
def send_rateless_det_doiter(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,maxiter): 
...
#R R/2 R/3 R/4.....		
def send_rateless_det_doiter_sim(N,T,compound_plist_u,channel_p,msg_length,doiter,runsim): #using bestchannel sent rate in place of derate
	...
	for i in range(runsim):
		...
		(achievedrate_sim,Iter,UN_msg_decoded)=send_rateless_det_doiter(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,doiter)
...
def send_rateless_det_Iter_retro_doiter(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,maxiter): 
...
def send_rateless_det_Iter_retro_doiter_sim(N,T,compound_plist_u,channel_p,msg_length,doiter,runsim): #using bestchannel sent rate in place of derate
	...
	for i in range(runsim):
		UN_msg=np.random.randint(2,size=Glist[0]-T)
		(achievedrate_sim,Iter,UN_msg_decoded)=send_rateless_det_Iter_retro_doiter(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,doiter-1)
		achievedrate+=float(achievedrate_sim)/runsim
		if UN_msg.tolist()!=UN_msg_decoded.tolist():
			block_errorcnt+=1
		try:
...
def send_rateless_det_Iter_retro_delta_doiter(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,deltaG,maxiter):
...
def send_rateless_det_Iter_retro_delta_doiter_sim(N,T,compound_plist_u,channel_p,msg_length,deltaG,doiter,runsim): #using bestchannel sent rate in place of derate
	...
	for i in range(runsim):
		...
		(achievedrate_sim,Iter,UN_msg_sent,UN_msg_decoded)=send_rateless_det_Iter_retro_delta_doiter(UN_msg,N,T,I_ord,channel_p,compound_plist,Glist,deltaG,doiter-1)
		achievedrate+=float(achievedrate_sim)/runsim
